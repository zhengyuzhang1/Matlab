function [meanx,meany,meanz]=meanxyz(w,dt,step,A,clust,eachclustn)
...
meanx=zeros(clustn,step);
meany=zeros(clustn,step);
meanz=zeros(clustn,step);
for t=2:step
    for i=1:clustn
        for j=1:eachclustn(i)
    ...
    for i=1:clustn
        ...
        meanx(i,t)=state(:,i,t)'*sxforclust(:,:,i)*state(:,i,t);
        meany(i,t)=state(:,i,t)'*syforclust(:,:,i)*state(:,i,t);
        meanz(i,t)=state(:,i,t)'*szforclust(:,:,i)*state(:,i,t);
    end
end
meanx=real(sum(meanx,1))/max(clust(:));
meany=real(sum(meany,1))/max(clust(:));
meanz=real(sum(meanz,1))/max(clust(:));
