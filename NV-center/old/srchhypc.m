function f = srchhypc(w, n, data, data0, t0, deltat, nuc)
...
while 1-mindv<0.5*(1-dps(find(dps==mindv)-1)) || (1-mindv)<0.5*(1-dps(find(dps==mindv)+1)) || (data(mid)>0 && (data(lp)<0.95 || data(rp)<0.95)) || mid-lp<2 || rp-mid<2 || abs(data(mid)-data(lp))/abs(data(mid)-data(rp))<0.5 || abs(data(mid)-data(lp))/abs(data(mid)-data(rp))>2 || (abs(data(mid)-data(mid-2))<0.015 && abs(data(mid)-data(mid+2))<0.015) || abs(data(mid)-data(mid+3))<0.02 || abs(data(mid)-data(mid-3))<0.02 || isnan(abs(data(mid)-data(lp))/abs(data(mid)-data(rp)))
...
if mindv>0.8
    ...
    prd=srchprd3(w,find(dips(:,2)==mindv),dips,tn);
elseif mindv>0.3
    dips=[t(locs(1-dps>0.3*(1-mindv))),dps(1-dps>0.3*(1-mindv))];
    prd=srchprd(w,find(dips(:,2)==mindv),dips,tn);
else
    dips=[t(locs(1-dps>0.5*(1-mindv))),dps(1-dps>0.5*(1-mindv))];
    prd=srchprd2(w,find(dips(:,2)==mindv),dips(:,1),tn);
end   
fitt=t(round((lp+mid)/2):round((mid+rp)/2));
if min([data(lp),data(rp)])>0.9
    fitdata=data(round((lp+mid)/2):round((mid+rp)/2))/min([data(lp),data(rp)]);
else
    fitdata=data(round((lp+mid)/2):round((mid+rp)/2));
end
...
if azx(2)<5e5
    range=abs(azx)*0.105;
    data1=data0;
    [~,gos]=fsrch(w,n,azx,[1,1],[1,1],data1,t0,deltat,tn);
end
if ~isempty(nuc)
