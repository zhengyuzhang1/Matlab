function [ OmGrad] = ThermoPot_grad(x,param,sin_kx,sin_ky,sin_kz,epsilonSq)
...
para = -8*g^2/(2*pi)^3* 2;
OmGrad(1)= para * Simp3D_sym(sin_kx.*uFactor,N,dk)  + 2*(gamma-2*mu)*u(1);
OmGrad(2)= para * Simp3D_sym(sin_ky.*uFactor,N,dk) + 2*(gamma-2*mu)*u(2);
OmGrad(3)= para * gamma_z * Simp3D_sym(sin_kz.*uFactor,N,dk) + 2*(gamma-2*mu + delta_z)*u(3);

OmGrad(4)= para * Simp3D_sym(sin_kx.*vFactor,N,dk)  + 2*(gamma-2*mu)*v(1);
OmGrad(5)= para * Simp3D_sym(sin_ky.*vFactor,N,dk)  + 2*(gamma-2*mu)*v(2);
OmGrad(6)= para *gamma_z * Simp3D_sym(sin_kz.*vFactor,N,dk)  + 2*(gamma-2*mu + delta_z)*v(3);
OmGrad(7) = dot(u,v);
%----------- Gradient of Om: OmGrad -----------% 
