...
cr2=0.99; % over condensation criterion: sqrt(u^2+v^2)>cr2, then phase = 4
cr3=10^-3; % criterion for isotropic px+ ipy: |uMag-vMag|/uMag < cr3, then phase 2

[varAll1, varAll2]=ndgrid([0.825:0.025:1.2], 2*mu+(1:0.05:10)); % two parameters are changed here; see below.
...
[kx, ky, kz]=ndgrid(-pi:dk:pi,-pi:dk:pi,0:dk:pi); % momentum space discretization
...
%  options_grad = optimoptions(@fsolve,'Display','off','Algorithm','levenberg-marquardt');
...
%     tic;
    for Ind2=1:size(varAll1,2)
        ...
        OmegaFunc=@(uv)ThermoPot_0T(uv,param,sin_kx,sin_ky,sin_kz,epsilonSq); %input with (ux,uy,uz,vx,vy,vz)
%         OmegaGradFunc = @(uv)ThermoPot_grad(uv,param,sin_kx,sin_ky,sin_kz,epsilonSq);
...
%         [orderParam,OmegaGrad]=fsolve(OmegaGradFunc,uvInit,options_grad);
...
OmegaAll_1 = OmegaAll; phaseAll_1 = phaseAll; 
uvMagAll_1 = uvMagAll; uvAll_1 = uvAll;
...
%         OmegaGradFunc = @(uv)ThermoPot_grad(uv,param,sin_kx,sin_ky,sin_kz,epsilonSq);
        ...
        % Sweeping along varAll1, i.e., use the orderParam for the previous run as the next initial condition
        if Ind2==size(varAll1,2) 
            if Ind1==size(varAll1,1)
                % The first point is obtained from random initial points
                % enforce the nonlinear constraint u^2+v^2<=1, and the gauge choice u.v==0.
                uvInit=orth(rand(3,2)); % generate two random orthgonal vectors with unit length.
                uvInit=[uvInit(:,1)*sqrt(0.5)*rand; uvInit(:,2)*sqrt(0.5)*rand]; % make the lengths of the vectors random.
...
%         [orderParam,OmegaGrad]=fsolve(OmegaGradFunc,uvInit,options_grad);
        ...
        if Omega < OmegaAll(Ind1, Ind2)
            ...
            OmegaAll(Ind1, Ind2)= Omega; 
            phaseAll(Ind1, Ind2)= phase; 
            uvMagAll(Ind1, Ind2)= uvMag; uvAll(Ind1,Ind2,:)= orderParam;
        end
...
clear kx ky kz sin_kx sin_ky sin_kz cos_kx cos_ky cos_kz epsilonSq OmegFunc
